...
class AnnotationProcessor:
    ...
    Parameters:
        ...
        yolo_annotations_path (str): путь к директории с аннотациями в формате YOLO .txt
        labelme_annotations_path (str): путь к директории с аннотациями в формате LabelMe .json
    ...
    def __init__(
            self,
            class_names: list,
            yolo_annotations_path: str,
            labelme_annotations_path: str,
            logger: logging.Logger
    ) -> None:
        self.class_names = class_names
        self.logger = logger
        self.yolo_annotations_path = yolo_annotations_path
        self.labelme_annotations_path = labelme_annotations_path
    ...
    def process_json_file(self, json_path: str) -> None:
        """
        Метод process_json_file() осуществляет конвертацию одного файла из
        формата LabelMe .json в формат YOLO .txt
        Parameters:
            json_path (str): полный путь к .json файлу
        Returns:
            None
        """
        try:
        ...
        image_path = os.path.join(self.labelme_annotations_path, image_filename)
        ...
        txt_path = os.path.join(self.yolo_annotations_path, base_name + ".txt")
    ...
    def convert_labelme_to_yolo(self) -> None:
        ...
        """
        if not os.path.isdir(self.labelme_annotations_path):
            self.logger.error(f"{self.labelme_annotations_path} is not a directory")
            raise NotADirectoryError(f"{self.labelme_annotations_path} is not a directory")

        if not os.path.isdir(self.yolo_annotations_path):
            self.logger.error(f"{self.yolo_annotations_path} is not a directory")
            raise NotADirectoryError(f"{self.yolo_annotations_path} is not a directory")

        os.makedirs(self.yolo_annotations_path, exist_ok=True)
        json_files = glob.glob(os.path.join(self.labelme_annotations_path, "*.json"))

        if len(json_files) == 0:
            self.logger.warning(f"There are no annotations in {self.labelme_annotations_path}")
            raise FileNotFoundError(f"There are no annotations in {self.labelme_annotations_path}")
